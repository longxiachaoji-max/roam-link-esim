            ...
            <div className="space-y-6 overflow-y-auto px-5 py-5">
              <section>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="text-xs text-white/50">優惠碼<input value={form.code} onChange={event => { const value = event.target.value; const isComposing = (event.nativeEvent as InputEvent).isComposing; setForm(current => ({ ...current, code: isComposing ? value : normalizeReferralCode(value) })); }} onCompositionEnd={event => { const value = normalizeReferralCode(event.currentTarget.value); setForm(current => ({ ...current, code: value })); }} onBlur={event => { const value = normalizeReferralCode(event.currentTarget.value); setForm(current => ({ ...current, code: value })); }} placeholder="例如 SUMMER2026" autoCapitalize="characters" autoCorrect="off" spellCheck={false} className="mt-1 h-11 w-full rounded-md border border-white/15 bg-black/25 px-3 font-mono text-white outline-none focus:border-cyan-400" /></label>
