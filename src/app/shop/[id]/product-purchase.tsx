...
export default function ProductPurchase({ product }: { product: PhysicalProduct }) {
  ...
  const rentalTotal = product.price * (rentalSelection?.days || 1);
  ...
  return <div className="mt-7 border-y border-black/10 py-6">
    ...
    {isRental && rentalSelection && <div className="mt-4 flex items-center justify-between rounded-md bg-[#dceee7] px-4 py-3"><div><p className="text-xs text-black/45">{rentalSelection.days} 天租金總額</p><p className="mt-1 text-xs text-black/40">每日 NT${product.price.toLocaleString()}</p></div><p className="text-2xl font-bold text-[#df4d5f]">NT${rentalTotal.toLocaleString()}</p></div>}
