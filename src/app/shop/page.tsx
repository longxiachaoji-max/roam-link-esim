  ...
  return (
    <main className="relative min-h-screen bg-[#f5f7f8] text-[#172028]">
      ...
      {checkoutOpen && <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/55 p-0 sm:items-center sm:p-4">
        <div role="dialog" aria-modal="true" aria-label="配送與付款" className="flex max-h-[94dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-md bg-white text-[#172028] shadow-2xl sm:rounded-md">
          <div className="flex shrink-0 items-center justify-between border-b border-black/8 px-5 py-4"><div><h2 className="text-xl font-bold">配送與付款</h2><p className="mt-1 text-xs text-black/40">登入會員：{sessionEmail}</p></div><button title="關閉" onClick={() => { setCheckoutOpen(false); setCheckoutMessage(''); }}><X size={20} /></button></div>
          ...
          <div className="shrink-0 border-t border-black/10 bg-white px-5 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          <div className="mb-3 flex items-center justify-between text-sm"><span className="font-semibold">應付總額</span><span className="text-xl font-bold text-[#df4d5f]">NT${total.toLocaleString()}</span></div>
          <div className="grid grid-cols-2 gap-2">{hasRental && deliveryMethod === 'pickup' && <button onClick={() => checkout('CASH_PICKUP')} disabled={paying !== null || !rentalTermsAccepted} className="flex h-11 items-center justify-center gap-2 rounded-md bg-[#247253] font-bold text-white shadow-sm disabled:opacity-40 col-span-2"><Banknote size={19} /> {paying === 'CASH_PICKUP' ? '送出面交申請中...' : '面交現場付款'}</button>}<button onClick={() => checkout('TOKENS')} disabled={paying !== null || tokenBalance === null || tokenBalance < total || (hasRental && !rentalTermsAccepted)} className="flex h-11 items-center justify-center gap-2 rounded-md bg-[#df4d5f] font-bold text-white disabled:bg-black/10 disabled:text-black/30"><WalletCards size={18} /> {paying === 'TOKENS' ? '立即扣款中...' : '儲值金立即付款'}</button><button onClick={() => checkout('Credit')} disabled={paying !== null || (hasRental && !rentalTermsAccepted)} className="flex h-11 items-center justify-center gap-2 rounded-md bg-[#172028] font-bold text-white disabled:opacity-40"><CreditCard size={18} /> {paying === 'Credit' ? '前往付款中...' : '信用卡付款'}</button>{!hasRental && <button onClick={() => checkout('BARCODE')} disabled={paying !== null} className="flex h-11 items-center justify-center gap-2 rounded-md bg-[#247253] font-bold text-white disabled:opacity-40 col-span-2"><Barcode size={18} /> {paying === 'BARCODE' ? '產生條碼中...' : '超商條碼付款'}</button>}</div>
