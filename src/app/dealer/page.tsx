...
import { ArrowLeft, Building2, ChevronRight, Copy, HandCoins, LogOut, Mail, Minus, Plus, RefreshCw, Send, WalletCards } from 'lucide-react';
...
interface Product {
  ...
  validity_days: number; retail_price: number; dealer_price?: number; commission_amount?: number;
}
...
interface ReferralCommission {
  id: string; code_snapshot: string; original_amount: number; paid_amount: number; item_count: number;
...
export default function DealerPage() {
  ...
  useEffect(() => {
    ...
    const refreshOrders = () => { if (dealer.sales_mode === 'referral') void loadReferrals(); else void loadOrders(); };
  ...
  const cartTotal = cartItems.reduce((sum, item) => sum + Number(item.dealer_price || 0) * item.quantity, 0);
  ...
  return (
    <main className="min-h-screen bg-[#090916] text-white">
                    ...
                    <div className="divide-y divide-white/8 border-y border-white/10">
                      {filteredProducts.map(product => (
                        <article key={product.id} className="grid gap-3 py-4 sm:grid-cols-[1fr_auto] sm:items-center">
                          ...
                          <div className="flex items-center justify-between gap-4 sm:justify-end"><div className="text-right"><p className="font-black text-[#55d5ea]">{money(product.dealer_price)}</p><p className="text-xs text-white/35">經銷價</p><p className="text-xs text-white/25 line-through">官網售價 {money(product.retail_price)}</p></div><button onClick={() => changeQuantity(product.id, 1)} title={`加入 ${product.name}`} className="grid size-10 place-items-center rounded-md bg-[#ff4f73]"><Plus size={20}/></button></div>
                        </article>
                      ))}
                    </div>
                  </div>
      ...
      {dealer.sales_mode !== 'referral' && view === 'sale' && cartItems.length > 0 && (
