alter table public.physical_products
  add column if not exists rental_free_shipping_days integer;

alter table public.physical_products
  drop constraint if exists physical_products_rental_free_shipping_days_check;
alter table public.physical_products
  add constraint physical_products_rental_free_shipping_days_check
  check (rental_free_shipping_days is null or rental_free_shipping_days between 1 and 365);

-- Preserve products whose existing public rental terms already promise this benefit.
update public.physical_products
set rental_free_shipping_days = 3
where category = 'rental'
  and rental_free_shipping_days is null
  and rental_terms like '%三天以上免運%';
