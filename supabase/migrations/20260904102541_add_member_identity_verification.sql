...
create index if not exists customer_identity_verifications_status_submitted_idx
  on public.customer_identity_verifications (status, submitted_at desc);

alter table public.customer_identity_verifications enable row level security;
revoke all on table public.customer_identity_verifications from anon, authenticated;
grant all on table public.customer_identity_verifications to service_role;
...
declare
  ...
  v_rental_free_shipping boolean := false;
  ...
  v_is_pickup_payment boolean;
begin
  if jsonb_typeof(p_items) <> 'array' or jsonb_array_length(p_items) = 0 then
    raise exception '購物車內容不正確';
  end if;
  if v_payment_method not in ('ECPAY_CREDIT', 'ECPAY_BARCODE', 'TOKENS', 'CASH_PICKUP') then
  ...
  v_is_pickup_payment := v_payment_method = 'CASH_PICKUP';

  if v_is_pickup_payment and v_delivery_method <> 'pickup' then
    raise exception '現場支付僅限預約面交';
  end if;
  ...
  loop
    ...
    if v_product.category = 'rental' then
      ...
      if v_product.rental_free_shipping_days is not null
        and v_rental_days >= v_product.rental_free_shipping_days
      then
        v_rental_free_shipping := true;
      end if;
  ...
  if v_is_pickup_payment and not v_has_rental then
    raise exception '現場支付目前僅開放租借商品';
  end if;
  if v_has_rental and coalesce((p_order->>'rental_terms_accepted')::boolean, false) is not true then
    raise exception '請先閱讀並同意租借合約條款';
  end if;
  ...
  insert into public.physical_orders (
    ...
    rental_terms_accepted_at, rental_terms_version, rental_terms_snapshot
  ) values (
    ...
    case when v_is_token_payment or v_is_pickup_payment then 'PROCESSING' else 'PENDING_PAYMENT' end,
    nullif(p_order->>'ecpay_trade_no', ''),
    case when v_is_token_payment then null else (p_order->>'reservation_expires_at')::timestamptz end,
    case when v_has_rental then now() else null end,
    case when v_has_rental then nullif(p_order->>'rental_terms_version', '') else null end,
    case when v_has_rental then nullif(p_order->>'rental_terms_snapshot', '') else null end
...
begin
  select * into v_order from public.physical_orders where id = p_order_id for update;
  ...
  loop
    ...
    select category into v_product_category from public.physical_products where id = v_item.product_id;
    if v_product_category = 'rental' then continue; end if;
