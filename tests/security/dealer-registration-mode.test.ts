...
test("registration stores only the requested mode and keeps commercial values admin-controlled", () => {
  const route = readFileSync(
    new URL("../../src/app/api/dealer/register/route.ts", import.meta.url),
    "utf8",
  );
  const page = readFileSync(
    new URL("../../src/app/dealer/page.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    route,
    /body\.referralCommission|body\.pricingValue|body\.referralDiscount/,
  );
...
test("referral applicants must choose an unused referral code before sign-up", () => {
  const route = readFileSync(
    new URL("../../src/app/api/dealer/register/route.ts", import.meta.url),
    "utf8",
  );
  const availabilityRoute = readFileSync(
    new URL(
      "../../src/app/api/dealer/referral-code-availability/route.ts",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  assert.match(
    route,
    /referral_code:\s*salesMode === ["']referral["'] \? referralCode : null/,
  );
  assert.match(route, /dealerReferralCodeIsAvailable/);
  assert.match(availabilityRoute, /此推薦碼已被使用，請更換其他推薦碼/);
  assert.match(page, /申請的推薦碼/);
  assert.match(page, /await checkReferralCodeAvailability\(\)/);
});
